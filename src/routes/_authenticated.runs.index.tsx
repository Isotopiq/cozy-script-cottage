...
import { useRuns, useScripts } from "@/lib/hooks/use-data";
      ...
      <Card className="overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-secondary/40">
            <tr>
              {["Script", "Started", "Duration", "Status"].map((h) => (
          ...
          <tbody>
                ...
                <tr key={r.id} className="border-t border-border hover:bg-secondary/20">
                  <td className="px-4 py-2.5">
                    <Link to="/runs/$id" params={{ id: r.id }} className="flex items-center gap-2 hover:text-primary">
                      ...
                      <span>{s?.name ?? r.script_id}</span>
                    </Link>
                  </td>
                  <td className="px-4 py-2.5 font-mono text-xs">{new Date(r.started_at).toLocaleString()}</td>
            ...
            {runs.length === 0 && (
              <tr><td colSpan={4} className="px-4 py-10 text-center text-xs text-muted-foreground">No runs yet.</td></tr>
            )}
          </tbody>
        </table>
      </Card>
